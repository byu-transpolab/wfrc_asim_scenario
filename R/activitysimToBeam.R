...
activitysim_output_dir <- "output_activitysim/full_no_RH"
beam_files_dir <- "data_beam/full_no_RH"

if(!dir.exists(beam_files_dir)) dir.create(beam_files_dir, recursive = T)
...
#should every household be given a bike?
has_bikes <- 1
...
persons %<>% 
	...
	rename(income = incomeValue) #needs to be `incomeValue` in hh, but `income` in
persons
...
hh_cars <- tibble(vehicleId = 1:nveh,
									 vehicleTypeId = "CAR",
									 householdId = veh_hh)
...
if(has_bikes){
	hh_bikes <- hh %>%
		select(householdId) %>%
		mutate(vehicleId = (nveh+1):(nveh+nrow(hh)),
			vehicleTypeId = "BIKE") %>%
		relocate(vehicleId, vehicleTypeId, householdId)

	vehicles <- bind_rows(hh_cars, hh_bikes)
