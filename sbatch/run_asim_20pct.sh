...
printf "Config path: configs_activitysim/20pct\n"
printf "Data path: data_activitysim\n"
printf "Output path: output_activitysim/20pct\n"

[[ ! -d "output_activitysim/20pct" ]] \
	&& mkdir -p output_activitysim/20pct

conda activate ASIM_DEV
conda info

#activitysim run --config configs_activitysim/20pct \
#	--data data_activitysim --output output_activitysim/20pct \
#	|| exit 1
python -m simulation --config configs_activitysim/20pct \
	--data data_activitysim --output output_activitysim/20pct \
	|| exit 1
