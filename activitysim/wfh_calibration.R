...
last_telecommute %>% 
  ...
  pivot_longer(adj_1:adj_4, names_to = "coef", values_to = "adj") %>% 
  select(pjobcat, coef, adj) %>% 
  mutate(coef = str_replace(coef, "adj_(\\d+)", "\\1day"))
...
prev_coeffs %>% 
  ...
  separate(coefficient_name, into = c("prefix", "coef", "days"))

new_coeffs_file <- file.path(cdir, paste0("telecommute_frequency_coefficients_", iter, ".csv"))
